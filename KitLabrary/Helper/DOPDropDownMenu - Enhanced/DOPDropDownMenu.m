...
@interface DOPDropDownMenu (){
    ...
    DOPIndexPath *_leftSelectIndex;
    
    NSInteger *_rightSelectRow;
...
//    CGPathRelease(bound);
    UIImage *image = [UIImage imageNamed:@"返回-1"];
...
- (void)animateIndicator:(CAShapeLayer *)indicator Forward:(BOOL)forward complete:(void(^)())complete {
    ...
    if (forward) {
        // 展开
        UIImage *image = [UIImage imageNamed:@"返回selected"];
        indicator.contents = (__bridge id _Nullable)(image.CGImage);
    } else {
        // 收缩
        UIImage *image = [UIImage imageNamed:@"返回-1"];
        indicator.contents = (__bridge id _Nullable)(image.CGImage);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_leftTableView == tableView) {
        ...
        _leftSelectIndex = [DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:index.row];
...
- (void)confiMenuWithSelectItem:(NSInteger)item {
    ...
    CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
    NSInteger currentSelectedMenudRow = [_currentSelectRowArray[_currentSelectedMenudIndex] integerValue];
    NSString * str = [_dataSource menu:self titleForItemsInRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:currentSelectedMenudRow item:item]];
    if ([str isEqualToString:@"全部"]) {
        str = [_dataSource menu:self titleForRowAtIndexPath:_leftSelectIndex];
    }
